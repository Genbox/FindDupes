//Modified xxHash64 implementation
...
namespace FindDupes.Hash
{
    public class xxHash : IFileHash
    {
        ...
        public ulong GetHash(in ReadOnlySpan<byte> buffer)
        {
            unchecked
            {
                ReadOnlySpan<byte> remainingBytes = buffer;
                ReadOnlySpan<QuadUlong> values = PopAll<QuadUlong>(ref remainingBytes);

                ulong h64 = !values.IsEmpty ? BulkStride(values) : PRIME64_5;

                h64 += (uint)buffer.Length;

                ReadOnlySpan<ulong> ulongSpan = PopAll<ulong>(ref remainingBytes);
                for (int i = 0; i < ulongSpan.Length; i++)
                {
                    ulong val = ulongSpan[i] * PRIME64_2;
        ...
        private static ulong BulkStride(in ReadOnlySpan<QuadUlong> input)
        {
            unchecked
            {
                ...
                {
                    ref readonly QuadUlong val = ref input[i];
        ...
        {
            ulong acc = RotateLeft(v1, 1) + RotateLeft(v2, 7) + RotateLeft(v3, 12) + RotateLeft(v4, 18);
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static ReadOnlySpan<T> PopAll<T>(ref ReadOnlySpan<byte> input) where T : struct
        {
            int totBytes = input.Length * Unsafe.SizeOf<byte>();
            int toLength = totBytes / Unsafe.SizeOf<T>();
            int sliceLength = toLength * Unsafe.SizeOf<T>() / Unsafe.SizeOf<byte>();

            ReadOnlySpan<T> result = MemoryMarshal.Cast<byte, T>(input);
